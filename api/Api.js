...
export const getByGenre = (id, page) => api.get(`discover/movie?api_key=${API_KEY}&with_genres=${id}&page=${page}`).then(res => res?.data?.results)
export const getMovies = (page, catagory ) => api.get(`movie/${catagory}?api_key=${API_KEY}&language=en-US&page=${page}`).then(res => res?.data?.results)
